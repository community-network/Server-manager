...
function AddIcon() {
  return (
    <svg viewBox="0 0 24 24" className={styles.sideIcon}>
...
export function Sidebar(props: {
  ...
  if (!isLoading) {
    if (!userError && user && user.auth.signedIn) {
      ...
      const manOptionsContent = (
        <>
          <span>{t("sidebar.manOptions")}</span>
          <SettingsIcon />
        </>
      );
      ...
      if (user.auth.isDeveloper) {
        devLink = (
          <PageLink
            ...
            name={t("sidebar.devOptions")}
            content={devOptionsContent}
          />
        );
      }
      ...
      if (user.auth.isManager) {
        manLink = (
          <PageLink
            to="/man/"
            callback={props.callback}
            name={t("sidebar.manOptions")}
            content={manOptionsContent}
          />
        );
      }
      ...
      groupLinks = [];
      for (const i in user.permissions.isAdminOf) {
        const group = user.permissions.isAdminOf[i];
        groupLinks.push(
          <PageLink
            callback={props.callback}
            to={"/group/" + group.id}
