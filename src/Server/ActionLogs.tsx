    ...
    <div className={styles.ActionLogs}>
      <div className={styles.logTitle}>
        ...
        <span value="log-density" className={styles.logDensity}>
          {logList
            ? t("server.console.amount", { amount: logList.logs.length })
            : ""}
        </span>
      </div>
...
function LogListing(props: { logList: ITailServerLog }): React.ReactElement {
  const { logList } = props;
  return (
...
function MenuArrowIcon(props: { down: boolean }): React.ReactElement {
  const { down } = props;
  return (
    <svg
      style={{ transform: down ? "rotateZ(180deg)" : "" }}
...
function LogRow(props: { log: ITailServerLogItem }): React.ReactElement {
  const { log } = props;
  const { t } = useTranslation();
...
function EmptyLogRow(): React.ReactElement {
